...
__export(constants_exports, {
  ...
  ROSETTANET_RPC_NODES: () => ROSETTANET_RPC_NODES,
  RPC_DEFAULT_VERSION: () => RPC_DEFAULT_VERSION,
  RPC_NODES: () => RPC_NODES,
  RosettaNetworkName: () => RosettaNetworkName,
  RosettanetChainId: () => RosettanetChainId,
...
var RosettaNetworkName = /* @__PURE__ */ ((RosettaNetworkName2) => {
  RosettaNetworkName2['MAIN'] = 'MAIN';
  return RosettaNetworkName2;
})(RosettaNetworkName || {});
var RosettanetChainId = /* @__PURE__ */ ((RosettanetChainId2) => {
  RosettanetChainId2['MAIN'] = '0x52535453';
  return RosettanetChainId2;
})(RosettanetChainId || {});
...
var FeeMarginPercentage = /* @__PURE__ */ ((FeeMarginPercentage2) => {
  ...
  FeeMarginPercentage2[(FeeMarginPercentage2['L1_BOUND_MAX_PRICE_PER_UNIT'] = 50)] =
...
var CallData = class _CallData {
  ...
  decodeParameters(typeCairo, response) {
    ...
    const decodedArray = typeCairoArray.map((typeParam) =>
      responseParser(responseIterator, { name: '', type: typeParam }, this.structs, this.enums)
    );
...
function estimateFeeToBounds(
  estimate,
  amountOverhead = 50 /* L1_BOUND_MAX_AMOUNT */,
  priceOverhead = 50 /* L1_BOUND_MAX_PRICE_PER_UNIT */
) {
  ...
  const maxUnits =
    estimate.data_gas_consumed !== void 0 && estimate.data_gas_price !== void 0
...
function toTransactionVersion(defaultVersion, providedVersion) {
  ...
  if (
    providedVersion &&
    !Object.values(api_exports.ETransactionVersion).includes(providedVersion0xs)
  ) {
...
function v3Details(details) {
  return {
    ...
    nonceDataAvailabilityMode:
      details.nonceDataAvailabilityMode || api_exports.EDataAvailabilityMode.L1,
    feeDataAvailabilityMode:
      details.feeDataAvailabilityMode || api_exports.EDataAvailabilityMode.L1,
...
var fetchPonyfill_default =
  (IS_BROWSER && window.fetch.bind(window)) || // use built-in fetch in browser if available
...
function isV3Tx(details) {
  ...
  return (
    version === api_exports.ETransactionVersion.V3 || version === api_exports.ETransactionVersion.F3
  );
}
...
var fromCallsToExecuteCalldata_cairo1 = (calls) => {
  const orderCalls = calls.map((call2) => ({
    ...
    calldata:
      Array.isArray(call2.calldata) && '__compiled__' in call2.calldata
        ? call2.calldata
        : CallData.compile(call2.calldata),
...
var RpcChannel = class {
  ...
  async getEstimateFee(
    invocations,
    { blockIdentifier = this.blockIdentifier, skipValidate = true }
  ) {
    const block_id = new Block(blockIdentifier).identifier;
    let flags = {};
...
function identifyRevision({ types, domain }) {
  if (
    revisionConfiguration[import_starknet_types_07.TypedDataRevision.ACTIVE].domain in types &&
    domain.revision === import_starknet_types_07.TypedDataRevision.ACTIVE
  )
    return import_starknet_types_07.TypedDataRevision.ACTIVE;
  if (
    revisionConfiguration[import_starknet_types_07.TypedDataRevision.LEGACY].domain in types &&
    (domain.revision ?? import_starknet_types_07.TypedDataRevision.LEGACY) ===
      import_starknet_types_07.TypedDataRevision.LEGACY
  )
...
function getDependencies(
  ...
  dependencies = [],
  contains = '',
  revision = import_starknet_types_07.TypedDataRevision.LEGACY
) {
...
function encodeType(types, type, revision = import_starknet_types_07.TypedDataRevision.LEGACY) {
  const allTypes =
    revision === import_starknet_types_07.TypedDataRevision.ACTIVE
  ...
  return newTypes
    .map((dependency) => {
      const dependencyElements = allTypes[dependency].map((t) => {
        const targetType =
          t.type === 'enum' && revision === import_starknet_types_07.TypedDataRevision.ACTIVE
            ? t.contains
            : t.type;
...
var RpcProvider = class {
  ...
  async waitForBlock(blockIdentifier = 'pending', retryInterval = 5e3) {
    ...
    const targetBlock =
      blockIdentifier === BlockTag.PENDING ? currentBlock + 1 : Number(toHex(blockIdentifier));
...
var LedgerSigner111 = class {
  ...
  async signTransaction(transactions, transactionsDetail) {
    ...
    if (Object.values(ETransactionVersion2).includes(transactionsDetail.version)) {
      ...
      Object.values(api_exports.ETransactionVersion3).includes(transactionsDetail.version)
    ) {
      const det = transactionsDetail;
      msgHash = calculateInvokeTransactionHash2({
        ...det,
...
var LedgerSigner221 = class extends LedgerSigner111 {
   ...
   */
  encodeCall(call2) {
    const toBuf = this.convertBnToLedger(call2.contractAddress);
    const selectorBuf = hexToBytes(addAddressPadding(getSelector(call2.entrypoint)));
    let calldataBuf = new Uint8Array([]);
    if (call2.calldata) {
      const compiledCalldata = CallData.compile(call2.calldata);
  ...
  async signTxV1(txDetails, calls) {
    ...
    const dataBuf = concatenateArrayBuffer([accountAddressBuf, maxFeeBuf, chainIdBuf, nonceBuf]);
    ...
    for (const call2 of calls) {
      const calldatas = this.encodeCall(call2);
...
var Account = class extends RpcProvider2 {
  ...
  constructor(
    ...
    pkOrSigner,
    cairoVersion,
    transactionVersion = config.get('accountTxVersion')
  ) {
  ...
  async getCairoVersion(classHash) {
    if (!this.cairoVersion) {
      const { cairo } = classHash
        ? await super.getContractVersion(void 0, classHash)
  ...
  async estimateDeclareFee(payload, details = {}) {
    ...
    const version = toTransactionVersion(
      !isSierra(payload.contract)
        ? api_exports.ETransactionVersion.F1
        : this.getPreferredVersion(
            api_exports.ETransactionVersion.F2,
            api_exports.ETransactionVersion.F3
          ),
      toFeeVersion(providedVersion)
    );
  ...
  async simulateTransaction(invocations, details = {}) {
    ...
    const accountInvocations = await this.accountInvocationsFactory(invocations, {
      ...
      versions: [
        ...
        toTransactionVersion(
          this.getPreferredVersion(
            api_exports.ETransactionVersion.V2,
  ...
  async getUniversalSuggestedFee(version, { type, payload }, details) {
    ...
    if (version === api_exports.ETransactionVersion.V3) {
      resourceBounds =
        ...
        (await this.getSuggestedFee({ type, payload }, details)).resourceBounds;
    } else {
      maxFee =
        details.maxFee ?? (await this.getSuggestedFee({ type, payload }, details)).suggestedMaxFee;
    }
    return {
      maxFee,
      resourceBounds,
  ...
  async buildInvocation(call2, details) {
    ...
    const signature = !details.skipValidate
      ? await this.signer.signTransaction(call2, details)
      : [];
    return {
      ...v3Details(details),
      contractAddress: this.address,
  ...
  async buildDeclarePayload(payload, details) {
    ...
    if (
      ...
      (details.version === api_exports.ETransactionVersion3.F3 ||
        details.version === api_exports.ETransactionVersion3.V3)
    ) {
...
// src/wallet/rosettanetAccount.ts
var import_rosettanet = require('rosettanet');

// src/wallet/rosettanetConnect.ts
...
function signMessage2(ewo, message, address) {
...
var RosettanetAccount = class _RosettanetAccount extends Account {
  ...
  constructor(providerOrOptions, walletProvider, cairoVersion, address = '') {
    ...
    if (!address.length) {
      ...
      requestAccounts2(this.walletProvider).then(([accountAddress]) => {
        this.address = accountAddress.toLowerCase();
      });
    }
  }
  ...
  async signMessage(message) {
    const evmSignedHash = await signMessage2(this.walletProvider, message, this.address);
    ...
    const r = cairo_exports.uint256(addHexPrefix(signedHashWithoutPrefix.slice(0, 63)));
    const s = cairo_exports.uint256(addHexPrefix(signedHashWithoutPrefix.slice(64, 127)));
    const v = addHexPrefix(signedHashWithoutPrefix.slice(128, 130));
    if (v !== '0x1c' && v !== '0x1b') {
      throw new Error('Invalid Ethereum Signature');
    }
    return [toHex(r.low), toHex(r.high), toHex(s.low), toHex(s.high), toHex(v)];
  }
  async execute(calls) {
    const txCalls = [].concat(calls).map((it) => {
      return {
  ...
  static async connect(provider, walletProvider, cairoVersion) {
    const [accountAddress] = await requestAccounts2(walletProvider);
